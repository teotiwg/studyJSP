...
<body>
	...
	<%
		request.removeAttribute("NoKeyValue");//키값이 틀려도 에러가 발생하지는 않음
	...
	새롭게 전달된 페이지의 내용을 출력하게된다. 
	그러므로 사용자는 페이지 이동을 인지하지 못하지만, 내부적으로 페이지전달(포워드)이
	되는 것이다. 쿼리스트링 형태로 파라미터도 전달할 수 있다. 
 	--%>
	...
	<%--
	sendRedirect()는 JS의 location.href와 동일한 기능으로 새로운 페이지에
	대한 요청을 하게된다. 이경우 request영역은 공유되지 않는다. 웹브라우저의
