...
<body>
	...
	\${htmlStr } : ${htmlStr }
  	...
  	<!--  
  	false일때는 HTML이 해석되어져서 화면에 출력된다. JS의 innerHTML()과
  	동일하다. 
  	-->
	...
	출력 : <c:out value="${null}" default="값이 null인 경우" />
 	...
 	-->
	<h3>페이지 링크에 응용하기</h3>
	<c:url value="/08Board/List.jsp?pageNum=" /><c:out value="${param.pageNum }" 
		default="1" />
	<hr />
	<!-- 주의]한줄로 붙여서 작성하세요. 줄바꿈하면 공백이 들어갑니다. -->	 
	<a href="<c:url value="/08Board/List.jsp?pageNum=" /><c:out value="${param.pageNum }" 
		default="1" />">회원제게시판 리스트 바로가기</a>
